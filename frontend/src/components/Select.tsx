...
const ButtonBase = styled.button<{ $open: boolean; $size: 'small' | 'normal' }>`
  ...
  padding: ${({ $size }) => ($size === 'small' ? '0.35rem 0.5rem' : '0.5rem 0.6rem')};
...
const OptionItem = styled.li<{ $selected: boolean; $focused: boolean; $size: 'small' | 'normal' }>`
  padding: ${({ $size }) => ($size === 'small' ? '0.35rem 0.5rem' : '0.45rem 0.6rem')};
  font-size: ${({ $size }) => ($size === 'small' ? '0.85rem' : '0.9rem')};
  border-radius: 6px;
  color: #0f172a;
  cursor: pointer;
...
export function Select<T extends string>({
  ...
  const toggle = () => {
    ...
    setFocusedIndex(Math.max(options.findIndex((option) => option.value === value), 0));
    setOpen(true);
  };
